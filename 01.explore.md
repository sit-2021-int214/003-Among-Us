...
#### code
```{R}
ImposterWin <- mean(amongUs$Team == "Imposter" & amongUs$ Outcome =="Win" )
AllImposter <- mean(amongUs$Team == "Imposter" )
...
```{R}
CrewmateWin <- mean(amongUs$Team == "Crewmate" & amongUs$ Outcome =="Win" )
AllCrewmate <- mean(amongUs$Team == "Crewmate" )
...
<Strong>ดังนั้น Crewmate มีโอกาศชนะมากกว่า</Strong>
```
